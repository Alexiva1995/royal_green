...
class PagoController extends Controller
{
	 ...
	 * @return view
	 */
	public function confimPrice($tipo){
		view()->share('title', 'Confirmar Pagos');
	    $moneda = Monedas::where('principal', 1)->get()->first();
		$pagos = $this->filtrarPagos($tipo, []);
		$fechas = [
			'desde' => '',
			'hasta' => ''
		];
	    return view('pagos.confirmarpago')->with(compact('pagos', 'fechas', 'moneda', 'tipo'));
	}
	...
	public function filtro(Request $datos)
	{
		...
		if (!empty($datos)) {
			...
			if ($datos->form == "confirmarpago") {
				if ($desde > $hasta) {
					return redirect('office/admin/price/confirmar/'.$datos->tipo)->with('msj2', 'La fecha desde no puede ser mayor que la fecha hasta');
				}
				...
				if ($desde > $hasta) {
					return redirect('office/admin/price/historial')->with('msj2', 'La fecha desde no puede ser mayor que la fecha hasta');
				}
	...
	/**
	 * Permite filtrar el pago por el tipo de activacion del usuario
	 *
	 * @param string $tipo
	 * @param array $fecha
	 * @return void
	 */
	public function filtrarPagos($tipo, $fecha)
	{
		if ($fecha == []) {
			$pagos = Pagos::where('estado', 0)->get();
		}else{
			$pagos = Pagos::where('estado', 0)->where('fechasoli', '>=', $fecha['desde'])->where('fechasoli', '<=', $fecha['hasta'])->get();
		}
		...
		foreach ($pagos as $pago) {
			...
			if ($verificarTipo != null) {
				if ($tipo == 'Real') {
					if ($verificarTipo->nivel_minimo_cobro == 0) {
						$newPagos->push($pago);
					}
				}else{
					if ($verificarTipo->nivel_minimo_cobro == 7) {
						$newPagos->push($pago);
					}
				}
			}
	...
	public function rechazarPago($id)
	{
		try {
			...
			$pagos->save();
			$user->save();
			return redirect()->back()->with('msj', 'Pago Rechazado Satisfactoriamente');
		} catch (\Throwable $th) {
